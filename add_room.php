...
<section class="contact py-5">
	<div class="container">
		<h2 class="heading text-capitalize mb-sm-5 mb-4"> Add Room </h2>
...
if(isset($_POST['add'])){
    ...
    $hostel_id = $_SESSION['hostel_id'];
    $allocate = 0;
    $query1 = "SELECT * FROM Room where Hostel_id = '$hostel_id' and Room_No = '$room_no'";
    $result1 = mysqli_query($conn,$query1);
    if(mysqli_num_rows($result1)==0){
        $query2 = "INSERT INTO Room(Hostel_id, Room_No, Allocated) VALUES('$hostel_id', '$room_no','$allocate')";
        $result = mysqli_query($conn,$query2);
        if($result){
            echo "<script type='text/javascript'>alert('Room Added Successfully!')</script>";
        }
        else{
                echo "<script type='text/javascript'>alert('Error in adding room!!! Please try again.')</script>";
        }
        
    }
    else{
        echo "<script type='text/javascript'>alert('Room is present. Add a new room')</script> ";
    }
}
?>
<br>
