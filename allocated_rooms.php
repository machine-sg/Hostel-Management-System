   ...
   if (isset($_POST['search'])) {
    ...
    <?php
   	   if(mysqli_num_rows($result_search)==0){
			...
			$roll_no1 = $row_search['Student_id'];
			$query3 = "SELECT * FROM Application WHERE Student_id = '$roll_no1'";
			$result3 = mysqli_query($conn,$query3);
			$row3 = mysqli_fetch_assoc($result3);
      		echo "<tr><td>{$student_name}</td><td>{$row_search['Student_id']}</td><td>{$row_search['Mob_no']}</td><td>{$hostel_name}</td><td>{$room_no}</td><td>{$row3['Payment_status']}</td></tr>\n";
   	   }
   }
   ?>
   </tbody>
      ...
      if(mysqli_num_rows($result1)==0){
         echo '<tr><td colspan="5">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
			...
			$roll_no = $row1['Student_id'];
			$query2 = "SELECT * FROM Application WHERE Student_id = '$roll_no'";
			...
			if($payment_status != 1){
      		...
      		echo "<tr><td>{$student_name}</td><td>{$row1['Student_id']}</td><td>{$row1['Mob_no']}</td><td>{$hostel_name}</td><td>{$room_no}</td><td>{$payment_status}</td></tr>\n";
      	}
      }
    ?>
    </tbody>
